...
//	int i ;
    ...
    if (!table) {
        ...
        table = calloc(1, sizeof(hash_table));
    	/* allocate memory for each bucket*/
        table->table = calloc(num_of_buckets, sizeof(counter*));
...
hash_table* double_table_size (hash_table* table, int num_of_buckets)
{
    ...
    hash_table* new_table = create_hash_table(NULL, num_of_buckets * 2);
    if (new_table) {
    
        int i;
        for (i=0; i<table->num_of_buckets; i++) {
            while (table->table[i]) {
                char * name = table->table[i]->hashtag;
 ...
 */
int add_to_table (hash_table* table, char* hashtag)
{
    if (table) {
    	/* determine proper bucket to put value into */
        unsigned int bucket_num = map(table, hashtag);
        counter** bucket_ptr = &table->table[bucket_num] ;

    	/* add value to proper bucket */
        *bucket_ptr = add(*bucket_ptr, hashtag);
    }
    return 0 ;
}
 ...
 * -returns 0 upon success
 */
int delete_from_table (hash_table* table, char* hashtag)
{

	/* determine proper bucket value is in */
    counter* search_result = find(table, hashtag);

	/* delete data from bucket's linked list */
    if (search_result) {
        int bucket_num = map(table, hashtag);
        counter* bucket = delete_node(table->table[bucket_num], hashtag);
...
counter* find (hash_table* table, char* hashtag) {
    ...
    if (table) {
    	...
    	return search_list(table->table[bucket_num], hashtag);
    } else return NULL;
}
...
counter* extract_max (hash_table* table) {
    counter* global_max = NULL;
    counter** max_bucket_ptr = NULL;
    
    //retrieve max counts from each bucket
    for (int i = 0; i < table->num_of_buckets; i++) {
        counter* local_max = get_max(table->table[i]);
    ...
    counter* max_copy = copy(global_max);
